...
const Hero = () => {
  const menuItems = [
    { label: "Cari Dokter", icon: <Stethoscope className="w-5 h-5 md:w-6 md:h-6 text-red-900" /> },
    { label: "Layanan Unggulan", icon: <Building2 className="w-5 h-5 md:w-6 md:h-6 text-red-900" /> },
    { label: "Medical Check Up", icon: <ClipboardList className="w-5 h-5 md:w-6 md:h-6 text-red-900" /> },
    { label: "Siloam at Home", icon: <Home className="w-5 h-5 md:w-6 md:h-6 text-red-900" /> },
    { label: "Lab", icon: <FlaskConical className="w-5 h-5 md:w-6 md:h-6 text-red-900" /> },
    { label: "Radiologi", icon: <MonitorPlay className="w-5 h-5 md:w-6 md:h-6 text-red-900" /> },
  ]
  ...
  return (
    <div className="w-full bg-white">
      ...
      <div className="hidden md:block relative w-full min-h-[650px] overflow-hidden">
        <Image src='/tes.avif' fill className='object-cover object-right' alt='Hero' priority />
          ...
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-6 gap-4">
              {menuItems.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-white rounded-xl shadow-xl border border-gray-50 group cursor-pointer hover:-translate-y-1 duration-300 transition-all">
      ...
      <div className="md:hidden flex flex-col">
        <div className="relative w-full aspect-[4/3]">
          <Image src='/tes.avif' fill className='object-cover' alt='Hero' />
        </div>
        
        <div className="px-4 py-6 bg-white">
          <p className="text-[10px] text-gray-500 uppercase tracking-wider">Selamat Datang di Siloam International Hospitals</p>
